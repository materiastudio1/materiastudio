...
const App = () => {
    ...
    const initCamera = async () => {
      try {
        ...
        const newStream = await navigator.mediaDevices.getUserMedia({
          video: { 
            facingMode,
            width: { ideal: 1920 }, // Prioritize high res
            height: { ideal: 1080 } 
          }
        });
        setStream(newStream);
  ...
  const takePhoto = () => {
    if (videoRef.current && canvasRef.current) {
        ...
        // Force portrait aspect ratio (9:16) 1080x1920
        ...
        if (ctx) {
            // "Object-cover" logic for drawing video to canvas
  ...
  return (
    // Outer container: Handles background, vertical centering for desktop, and full height
    <div className="w-full h-[100dvh] bg-black text-white flex items-center justify-center overflow-hidden font-sans">
        
        {/* Main App Container: Simulates mobile view on desktop, full size on mobile */}
        <div className="relative w-full h-full sm:max-w-[480px] sm:h-[95dvh] sm:rounded-3xl sm:border-4 sm:border-gray-800 bg-gray-900 overflow-hidden shadow-2xl">
            ...
            {/* Controls Layer: Handles SafeArea and Positioning */}
            <div 
            ...
            >
                ...
                {/* Footer Controls */}
                <div className="pointer-events-auto flex flex-col gap-6 items-center w-full">
                    
                    {/* Frame Selector (Camera Mode) */}
                    {mode === AppMode.CAMERA && (
                        <>
                            {/* Scroll Container */}
                            <div className="w-full overflow-x-auto flex items-center gap-4 px-2 no-scrollbar justify-center snap-x overscroll-x-contain pb-2">
                                {frames.map(frame => (
                                    <button
                                        key={frame.id}
                                        onClick={() => setActiveFrame(frame)}
                                        className={`
                                            w-[64px] h-[64px] flex-shrink-0 rounded-full border-2 overflow-hidden relative transition-all snap-center shadow-lg
                    ...
                    {/* Preview Actions (Preview Mode) */}
                    {mode === AppMode.PREVIEW && (
                        <div className="flex items-center justify-center gap-6 pb-4 w-full">
                            <GlassButton 
